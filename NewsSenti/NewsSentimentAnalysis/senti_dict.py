...
import re
#打开词典文件，返回列表
def open_dict(Dict = 'name', path=r'./Sent_Dict/'):
...
deny_word = open_dict(Dict = '否定词', path= r'./Sent_Dict/')
posdict = open_dict(Dict = 'positive', path= r'./Sent_Dict/')
negdict = open_dict(Dict = 'negative', path= r'./Sent_Dict/')
degree_word = open_dict(Dict = '程度级别词语', path= r'./Sent_Dict/')
...
def Senti_Text(text):
    ...
    for sentence in seg_sentence:
        if sentence != '':
            pos,neg,res=Senti_Sentence(sentence)
            pos_sum+=pos
            neg_sum+=neg
            sen_num+=1
        else:
